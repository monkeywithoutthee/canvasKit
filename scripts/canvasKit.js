    ...
    const handleMouseDown = ((event)=>{
      ...
      //  ctxo.strokeText('booom', 10, 50);
        ...
        if (oPage.currentOption==='grade'){
          const selCol = document.getElementById('colPickr').value;
          if (!oPage.isDown){
            ctxo.beginPath();
          };
          if (oPage.isPathStarted){
            oPage.images[oPage.currentImage].imagedata.push({type:'grade',move:{x:oPage.lastPoint.x,y:oPage.lastPoint.y},firstmove:false,colour:selCol});
            ctxo.lineTo(oPage.lastPoint.x,oPage.lastPoint.y);
          };
          oPage.images[oPage.currentImage].imagedata.push({type:'grade',move:{x:oPage.startX,y:oPage.startY},firstmove:!oPage.isPathStarted,colour:selCol});
    ...
    const handleMouseUp = ((event)=>{
        ...
        if (oPage.currentOption==='square'){
          ...
          oPage.images[oPage.currentImage].imagedata.push({type:'square',move:{x:oPage.prevStartX,y:oPage.prevStartY,width:oPage.prevWidth,height:oPage.prevHeight},firstmove:true,colour:selCol});
        ...
        if (oPage.currentOption==='circle'){
          ...
          if (oPage.prevWidth/2>1){
                    oPage.images[oPage.currentImage].imagedata.push({type:'circle',move:{x:oPage.prevStartX *1+(oPage.prevWidth/2),y:oPage.prevStartY *1+(oPage.prevWidth/2),radius:oPage.prevWidth/2,starta:0,enda:360},firstmove:true,colour:selCol});
            ctxo.beginPath();
    ...
    //  console.log(imageData,'<<drawstuff::', oPage.images[oPage.currentImage].imagedata);
        imageData.forEach((item, i) => {
          if (item.move){
            ...
            ctx.strokeStyle = item.colour || '#000000';
            ctxo.strokeStyle = item.colour || '#000000';
            if (item.type==='square'){
